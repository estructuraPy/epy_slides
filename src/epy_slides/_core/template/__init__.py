...
_AUTOFIT_FN = (
    ...
    "  var deck = window._epyDeck, H = 0;\n"
    "  try { H = (deck && deck.getConfig && deck.getConfig().height) || 0; }\n"
    "  catch (e) { H = 0; }\n"
    ...
    "    // Measure against the fixed slide-frame height (the reveal config\n"
    "    // height), not the section's clientHeight: on screen the section\n"
    "    // grows to its content (no print-mode pin) so clientHeight would\n"
    "    // never report an overflow and dense slides would clip.\n"
    "    var box = H || sec.clientHeight;\n"
    "    var avail = box - pad;\n"
...
def build_reveal_document(
    ...
    *,
    ...
    init = (
        ...
        "      document.querySelectorAll('.reveal a[href]').forEach(function(a){\n"
